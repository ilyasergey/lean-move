  ...
  | some val1 =>
    ...
    | some val2 =>
      ...
      | some val3 =>
        ...
        · -- (I,V,V): extract bt from hve1, compose with hve2
          obtain ⟨⟨bt, rfl, rfl⟩, _⟩ := hve1
          simp [applySubstMoveType] at hve2
          exact ⟨bt, rfl, hve2.1.symm⟩
        · -- (I,I,V): extract bt from hve2, use baseCompatible with hve1
          obtain ⟨⟨bt, rfl, rfl⟩, _⟩ := hve2
          cases τ1 with
          | basic bt1 =>
            simp [MoveType.baseCompatible] at hve1
            have hbt := hve1.1
            subst hbt
            exact ⟨_, rfl, rfl⟩
          | ref => simp [MoveType.baseCompatible] at hve1
        · -- (I,I,I): transitivity
...
/-- Extract invalid var info from VarEnvSubstEquiv.
    When ve1 has invalidVar, ve2 may have invalidVar or validVar (both with baseCompatible). -/
private lemma VarEnvSubstEquiv_lookup_invalid (σ : Aref → Aref) (ve1 ve2 : VarEnv) (x : Var)
    (τ : MoveType) (ms : Mut) :
    VarEnvSubstEquiv σ ve1 ve2 →
    lookup ve1 x = some (.invalidVar, τ, ms) →
    ∃ isv2 τ2, lookup ve2 x = some (isv2, τ2, ms) ∧ MoveType.baseCompatible τ τ2 := by
  intro hve hlook
  unfold VarEnvSubstEquiv at hve
  specialize hve x
  simp only [hlook] at hve
  ...
  | some val2 =>
    ...
    | invalidVar => simp_all
    | validVar =>
      obtain ⟨⟨bt, hτ1, hτ2⟩, hms⟩ := hve
      subst hτ1; subst hτ2
      exact ⟨.validVar, .basic bt, by simp_all, by simp [MoveType.baseCompatible]⟩
...
theorem TypeEnv.subsumes_trans (envL1 envL2 env : TypeEnv) :
  ...
  obtain ⟨hnonroot1, hpaths1, henum_equiv1⟩ := hlast1
  obtain ⟨σ2, hid2, hve2, hse2, hrefs2, hinj2, hlast2⟩ := hsub2
  obtain ⟨hnonroot2, hpaths2, henum_equiv2⟩ := hlast2
  ...
  · constructor
    ...
    · -- paths ∧ LookupEquiv
      ...
      · intro k; rw [henum_equiv1 k, henum_equiv2 k]
...
lemma SiteEnvSubstEquiv_deleteAll (σ : Aref → Aref) (se1 se2 : SiteEnv) (keys : List Site) :
    SiteEnvSubstEquiv σ se1 se2 →
...
/-- SiteEnvSubstEquiv is preserved by addVecSites (only basic types inserted) -/
private lemma SiteEnvSubstEquiv_addVecSites (σ : Aref → Aref)
    (T : BasicMoveType)
    (se1 se2 : SiteEnv) (sites : List Site) :
    SiteEnvSubstEquiv σ se1 se2 →
    SiteEnvSubstEquiv σ (addVecSites T se1 sites) (addVecSites T se2 sites) := by
  intro hse
  induction sites generalizing se1 se2 with
  | nil => exact hse
  | cons hd rest ih =>
    simp only [addVecSites]
    apply ih
    exact SiteEnvSubstEquiv_insert σ _ _ _ _ _ hse (applySubstMoveType_basic σ T)

/-- site_tracked is preserved by addVecSites (only basic types inserted) -/
private lemma site_tracked_addVecSites (pe_refs : List Aref)
    (T : BasicMoveType)
    (se : SiteEnv) (sites : List Site)
    (hst : ∀ s bt r bk, lookup se s = some (.ref bt r bk) → r ∈ pe_refs) :
    ∀ s bt r bk, lookup (addVecSites T se sites) s = some (.ref bt r bk) → r ∈ pe_refs := by
  induction sites generalizing se with
  | nil => exact hst
  | cons hd rest ih =>
    simp only [addVecSites]
    apply ih
    exact site_tracked_insert_basic _ _ _ _ hst
...
lemma RefsUnique_deleteAll (ve : VarEnv) (se : SiteEnv) (keys : List Site)
...
/-- RefsUnique is preserved by addVecSites (only basic types inserted) -/
private lemma RefsUnique_addVecSites (ve : VarEnv) (se : SiteEnv)
    (T : BasicMoveType) (sites : List Site)
    (h : RefsUnique ve se) :
    RefsUnique ve (addVecSites T se sites) := by
  induction sites generalizing se with
  | nil => exact h
  | cons hd rest ih =>
    simp only [addVecSites]
    apply ih
    exact RefsUnique_insert_basic _ _ _ _ h
...
private theorem weaken_let_bind_borrowMut
  ...
  apply typecheck_stmt.let_bind_borrowMut lenv env _ _ _ _ r' _ _
    hle hlook_env (SiteEnvSubstEquiv_notIn σ _ _ _ hse hnotIn) hr'_fresh_prop
  apply ih
  · -- subsumes
    rw [uwe_absorbs_epsilon r .root _ envL.pathEnv hr_fresh_pe (Ne.symm hr_ne_root),
        uwe_absorbs_epsilon r' .root _ env.pathEnv hr'_fresh_pe (Ne.symm hr'_not_root)]
    ...
    · have := path_inclusion_update_with_extension σ _ _ r r' .root
        [.root_to_var x_var] hpaths hr_fresh_pe hr'_not_mapped hroot
      rw [hσ_root] at this; exact ⟨this, henum_equiv⟩
  · exact hfuneq
  · exact ⟨update_with_extension_wellformed r .root [.root_to_var x_var]
            (update_with_epsilon r r envL.pathEnv)
            (update_with_epsilon_wellformed r r envL.pathEnv hwfL.pathEnv_wf hr_ne_root)
  ...
  · -- subsumes with extendSubst σ rf rf'
    ...
    · -- path_inclusion ∧ LookupEquiv
      exact ⟨path_inclusion_update_with_extension σ _ _ rf rf' s [.field f]
        hpaths hrf_fresh_pe hrf'_not_mapped hs_mem, henum_equiv⟩
  · exact hfuneq
  · -- WellFormed envL'
  ...
  · -- subsumes with extendSubst σ r' r''
    ...
    · -- path_inclusion ∧ LookupEquiv
      exact ⟨path_inclusion_consume_ref_transfer σ envL.pathEnv env.pathEnv r r' r''
        ...
        hwfL.pathEnv_wf.self_loop_accepts_nil, henum_equiv⟩
  · exact hfuneq
  · -- WellFormed envL'
...
private theorem weaken_let_bind_intLit
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := insert envL.siteEnv a (.basic (.int w))}
        cont retTypes) :
...
private theorem weaken_var_assign_invalid
    ...
    typecheck_stmt lenv env (.assign x a cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  obtain ⟨isv2, τ_env, hlook_x_env, hbc⟩ := VarEnvSubstEquiv_lookup_invalid σ _ _ _ _ _ hve hlook_x
  have hlook_a_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_a
  ...
  | invalidVar =>
    ...
    · -- IH: continuation env subsumes
      ...
      · exact hroot
  | validVar =>
    -- Extract basic type constraint from VarEnvSubstEquiv (I, V) restriction
    have hve_copy := hve
    unfold VarEnvSubstEquiv at hve_copy
    have hraw := hve_copy x
    simp only [hlook_x, hlook_x_env] at hraw
    obtain ⟨⟨bt, hτ_basic, hτ_env_basic⟩, _⟩ := hraw
    subst hτ_basic; subst hτ_env_basic
    -- MoveType.compatible (.basic bt) τ' forces τ' = .basic bt
    have hτ'_basic : τ' = .basic bt := by
      cases τ' with
      | basic bt' => simp [MoveType.compatible] at hcompat; exact congrArg _ hcompat.symm
      | ref => simp [MoveType.compatible] at hcompat
    subst hτ'_basic
    simp only [applySubstMoveType] at hlook_a_env
    -- Use the new var_assign_overwrite_basic rule
    apply typecheck_stmt.var_assign_overwrite_basic lenv env _ _ .validVar bt _ _
      hlook_x_env hlook_a_env
    -- IH: continuation env subsumes
    apply ih
    · exact ⟨σ, hid,
    ...
    · exact ⟨hwfL.pathEnv_wf,
            SiteEnv.delete_refs_not_root _ _ hwfL.siteEnv_wf,
            VarEnv.update_refs_not_root _ _ _ hwfL.varEnv_wf (by simp)⟩
    · exact ⟨hwfE.pathEnv_wf,
            SiteEnv.delete_refs_not_root _ _ hwfE.siteEnv_wf,
            VarEnv.update_refs_not_root _ _ _ hwfE.varEnv_wf (by simp)⟩
    · exact site_tracked_delete _ _ _ hsite_tracked
    · exact var_tracked_update_valid_from_site _ _ _ _ _ _ _ hlook_a hsite_tracked hvar_tracked
    · exact RefsUnique_assign_from_site _ _ _ _ _ _ hlook_a huniq
    · exact hpaths_to_nm
...
private theorem weaken_release_basic
    (lenv : LabelEnv) (envL env : TypeEnv)
    (a : Site) (bt : BasicMoveType)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_a : lookup envL.siteEnv a = some (.basic bt))
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    (hsite_tracked : ∀ s bt r bk, lookup envL.siteEnv s = some (.ref bt r bk) → r ∈ envL.pathEnv.refs)
  ...
  have hlook_a_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_a
  simp only [applySubstMoveType] at hlook_a_env
  apply typecheck_stmt.release_basic lenv env _ _ _ _ hlook_a_env
  apply ih
  · exact ⟨σ, hid, hve, SiteEnvSubstEquiv_delete σ _ _ _ hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩
  · exact hfuneq
  · exact ⟨hwfL.pathEnv_wf, SiteEnv.delete_refs_not_root _ _ hwfL.siteEnv_wf, hwfL.varEnv_wf⟩
  · exact ⟨hwfE.pathEnv_wf, SiteEnv.delete_refs_not_root _ _ hwfE.siteEnv_wf, hwfE.varEnv_wf⟩
  · intro s' bt' r' bk' hlook_s'
    by_cases hs' : s' = a
    · subst hs'; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
    · rw [lookup_delete_ne _ _ _ hs'] at hlook_s'
...
private theorem weaken_var_assign_valid_ref
    (lenv : LabelEnv) (envL env : TypeEnv)
    (x : Var) (a : Site)
    (τ_old : BasicMoveType) (r_old : Aref) (bk_old : BorrowingKind) (τ' : MoveType) (ms : Mut)
    (cont : Stmt) (retTypes : List ParamType)
    (hle : LE.le .mutable ms)
    (hlook_x : lookup envL.varEnv x = some (.validVar, .ref τ_old r_old bk_old, ms))
    (hlook_a : lookup envL.siteEnv a = some τ')
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with varEnv := update envL.varEnv x (.validVar, τ', ms)
                   siteEnv := delete envL.siteEnv a
                   pathEnv := delete_ref_node envL.pathEnv r_old}
        cont retTypes) :
    typecheck_stmt lenv env (.assign x a cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  -- Translate lookups through σ
  have hlook_x_env := VarEnvSubstEquiv_lookup_valid σ _ _ _ _ _ hve hlook_x
  simp only [applySubstMoveType] at hlook_x_env
  have hlook_a_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_a
  have hτ'_nr := hwfL.siteEnv_wf _ _ hlook_a
  -- r_old facts
  have hr_mem := hvar_tracked _ _ _ _ _ hlook_x
  have hr_ne_root := hwfL.varEnv_wf _ _ hlook_x
  have hσr_ne_root : σ r_old ≠ .root := hnonroot r_old hr_ne_root
  -- Apply var_assign_valid_ref rule for env
  apply typecheck_stmt.var_assign_valid_ref lenv env _ _ _ (σ r_old) _ _ _ _ _
    hle hlook_x_env hlook_a_env
  -- IH: continuation
  apply ih
  · -- subsumes: σ, delete_ref_node on both sides, update varEnv, delete siteEnv
    refine ⟨σ, hid,
      VarEnvSubstEquiv_update_valid σ _ _ _ _ _ hve,
      SiteEnvSubstEquiv_delete σ _ _ _ hse,
      ?_, ?_, hnonroot, ?_⟩
    · -- refs
      simp only [delete_ref_node]
      rw [map_filter_ne σ _ r_old hinj hr_mem]
      exact hrefs.filter _
    · -- injectivity
      intro u v hu hv
      simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq] at hu hv
    ...
    · -- path_inclusion ∧ LookupEquiv
      ...
      · intro u v hu hv path hinterp
        simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq] at hu hv
        have hσu_ne : σ u ≠ σ r_old := fun h => hu.2 (hinj u r_old hu.1 hr_mem h)
        have hσv_ne : σ v ≠ σ r_old := fun h => hv.2 (hinj v r_old hv.1 hr_mem h)
        simp only [delete_ref_node] at hinterp ⊢
        rw [if_neg (fun ⟨h, _⟩ => hu.2 h), if_neg (not_or.mpr ⟨hu.2, hv.2⟩)]
        rw [if_neg (fun ⟨h, _⟩ => hσu_ne h), if_neg (not_or.mpr ⟨hσu_ne, hσv_ne⟩)] at hinterp
        exact hpaths u v hu.1 hv.1 path hinterp
  ...
  · -- WellFormed modified env
    exact ⟨delete_ref_node_wellformed _ _ hwfE.pathEnv_wf hσr_ne_root,
           ...
           VarEnv.update_refs_not_root _ _ _ hwfE.varEnv_wf
             (moveTypeRefNotRoot_applySubst σ _ hτ'_nr hnonroot)⟩
  · -- site_tracked: siteEnv = delete envL.siteEnv a, pathEnv = delete_ref_node r_old
    intro s' bt' r' bk' hlook_s'
    simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq]
    by_cases hs' : s' = a
    · subst hs'; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
    · rw [lookup_delete_ne _ _ _ hs'] at hlook_s'
      exact ⟨hsite_tracked _ _ _ _ hlook_s',
        fun hr'_eq => by rw [hr'_eq] at hlook_s'; exact (huniq r_old).1 x _ _ _ s' _ _ hlook_x hlook_s'⟩
  · -- var_tracked: varEnv = update env.varEnv x (.validVar, τ', ms), pathEnv = delete_ref_node r_old
    intro x' bt' r' bk' ms' hlook_x'
    have hlook_x'' : lookup (insert envL.varEnv x (.validVar, τ', ms)) x' =
        some (.validVar, .ref bt' r' bk', ms') := hlook_x'
    simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq]
    by_cases hx' : x' = x
    · subst hx'; rw [lookup_insert_same] at hlook_x''
      have hτ : τ' = .ref bt' r' bk' := by
        simp only [Option.some.injEq, Prod.mk.injEq] at hlook_x''; exact hlook_x''.2.1
      rw [hτ] at hlook_a
      refine ⟨hsite_tracked _ _ _ _ hlook_a, ?_⟩
      intro hr'_eq; rw [hr'_eq] at hlook_a
      exact (huniq r_old).1 x' _ _ _ a _ _ hlook_x hlook_a
    · rw [lookup_insert_ne _ _ _ _ hx'] at hlook_x''
      refine ⟨hvar_tracked _ _ _ _ _ hlook_x'', ?_⟩
      intro hr'_eq; rw [hr'_eq] at hlook_x''
      exact (huniq r_old).2.2 x x' _ _ _ _ _ _ (Ne.symm hx') hlook_x hlook_x''
  · -- RefsUnique: update varEnv x, delete siteEnv a
    exact RefsUnique_assign_from_site _ _ _ _ _ _ hlook_a huniq
  · exact delete_ref_node_paths_to_non_member _ _ hpaths_to_nm
  · exact delete_ref_node_paths_from_non_member _ _ hpaths_from_nm
  · exact self_loop_only_empty_drn env.pathEnv _ hself_loop_only_empty
...
private theorem weaken_let_bind_copy_ref
  ...
  · -- subsumes with extendSubst σ t t'
    ...
    · -- path_inclusion ∧ LookupEquiv
      exact ⟨path_inclusion_update_with_extension σ _ _ t t' s [] hpaths ht_fresh_pe ht'_not_mapped hs_mem, henum_equiv⟩
  · exact hfuneq
  · -- WellFormed envL'
...
-- ============================================================
-- Helper lemmas for vector operation weakening
-- ============================================================

private theorem weaken_let_bind_vecPack
    (lenv : LabelEnv) (envL env : TypeEnv)
    (b : Site) (T : BasicMoveType) (elems : List Site)
    (cont : Stmt) (retTypes : List ParamType)
    (hnotIn : notIn envL.siteEnv b)
    (hforall : ∀ a, a ∈ elems → lookup envL.siteEnv a = some (.basic T))
    (hnodup : List.Nodup elems)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    (hsite_tracked : ∀ s bt r bk, lookup envL.siteEnv s = some (.ref bt r bk) → r ∈ envL.pathEnv.refs)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := insert (deleteAll envL.siteEnv elems) b (.basic (.tvec T))}
        cont retTypes) :
    typecheck_stmt lenv env (.letBind b (.vecPack T elems) cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  apply typecheck_stmt.let_bind_vecPack
  · exact SiteEnvSubstEquiv_notIn σ _ _ _ hse hnotIn
  · intro a hmem
    have hlook_a := hforall a hmem
    have hlook_a_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_a
    simp only [applySubstMoveType] at hlook_a_env
    exact hlook_a_env
  · exact hnodup
...
private theorem weaken_vecUnpack_rule
    (lenv : LabelEnv) (envL env : TypeEnv)
    (T : BasicMoveType) (results : List Site) (src : Site)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook : lookup envL.siteEnv src = some (.basic (.tvec T)))
    (hfresh : ∀ a, a ∈ results → notIn envL.siteEnv a)
    (hnodup : List.Nodup results)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    (hsite_tracked : ∀ s bt r bk, lookup envL.siteEnv s = some (.ref bt r bk) → r ∈ envL.pathEnv.refs)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := addVecSites T (delete envL.siteEnv src) results}
        cont retTypes) :
    typecheck_stmt lenv env (.vecUnpack T results src cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  have hlook_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook
  simp only [applySubstMoveType] at hlook_env
  apply typecheck_stmt.vecUnpack_rule lenv env _ _ _ _ _
    hlook_env
  · intro a hmem
    exact SiteEnvSubstEquiv_notIn σ _ _ _ hse (hfresh a hmem)
  · exact hnodup
  · apply ih
    · exact ⟨σ, hid, hve,
        SiteEnvSubstEquiv_addVecSites σ _ _ _ _
          (SiteEnvSubstEquiv_delete σ _ _ _ hse),
        hrefs, hinj, hnonroot, hpaths, henum_equiv⟩
    · exact hfuneq
    · exact ⟨hwfL.pathEnv_wf,
        addVecSites_refs_not_root _ _ _
          (SiteEnv.delete_refs_not_root _ _ hwfL.siteEnv_wf),
        hwfL.varEnv_wf⟩
    · exact ⟨hwfE.pathEnv_wf,
        addVecSites_refs_not_root _ _ _
          (SiteEnv.delete_refs_not_root _ _ hwfE.siteEnv_wf),
        hwfE.varEnv_wf⟩
    · exact site_tracked_addVecSites _ _ _ _
        (site_tracked_delete _ _ _ hsite_tracked)
    · exact hvar_tracked
    · exact RefsUnique_addVecSites _ _ _ _ (RefsUnique_delete_site _ _ _ huniq)
    · exact hpaths_to_nm
    · exact hpaths_from_nm
    · exact hself_loop_only_empty
    · exact hroot

private theorem weaken_let_bind_vecLen
    (lenv : LabelEnv) (envL env : TypeEnv)
    (a src : Site) (T : BasicMoveType) (r : Aref) (isBor : BorrowingKind)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_src : lookup envL.siteEnv src = some (.ref (.tvec T) r isBor))
    (hnotIn : notIn envL.siteEnv a)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := insert (delete envL.siteEnv src) a (.basic .u64)
                   pathEnv := delete_ref_node envL.pathEnv r}
        cont retTypes) :
    typecheck_stmt lenv env (.letBind a (.vecLen src) cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  have hlook_src_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_src
  simp only [applySubstMoveType] at hlook_src_env
  have hr_mem := hsite_tracked _ _ _ _ hlook_src
  have hr_ne_root := hwfL.siteEnv_wf _ _ hlook_src
  have hσr_ne_root : σ r ≠ .root := hnonroot r hr_ne_root
  apply typecheck_stmt.let_bind_vecLen lenv env _ _ _ _ _ _ _
    hlook_src_env
    (SiteEnvSubstEquiv_notIn σ _ _ _ hse hnotIn)
  apply ih
  · -- subsumes
  ...
  · -- site_tracked
    ...
    simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq]
    by_cases hs'a : s' = a
    · subst hs'a; rw [lookup_insert_same] at hlook_s'; cases hlook_s'
    · rw [lookup_insert_ne _ _ _ _ hs'a] at hlook_s'
      by_cases hs'src : s' = src
      · subst hs'src; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
      · rw [lookup_delete_ne _ _ _ hs'src] at hlook_s'
        exact ⟨hsite_tracked _ _ _ _ hlook_s',
          fun hr'_eq => by rw [hr'_eq] at hlook_s'; exact (huniq r).2.1 s' src _ _ _ _ hs'src hlook_s' hlook_src⟩
  · -- var_tracked
    intro x' bt' r' bk' ms' hlook_x'
    simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq]
    exact ⟨hvar_tracked _ _ _ _ _ hlook_x',
      fun hr'_eq => by rw [hr'_eq] at hlook_x'; exact (huniq r).1 x' _ _ _ src _ _ hlook_x' hlook_src⟩
  · exact RefsUnique_insert_basic _ _ _ _ (RefsUnique_delete_site _ _ _ huniq)
  · exact delete_ref_node_paths_to_non_member _ _ hpaths_to_nm
  · exact delete_ref_node_paths_from_non_member _ _ hpaths_from_nm
  · exact self_loop_only_empty_drn env.pathEnv _ hself_loop_only_empty
...
private theorem weaken_let_bind_vecImmBorrow
    (lenv : LabelEnv) (envL env : TypeEnv)
    (a src idx : Site) (T : BasicMoveType) (s rf : Aref) (isBor : BorrowingKind)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_src : lookup envL.siteEnv src = some (.ref (.tvec T) s isBor))
    (hlook_idx : lookup envL.siteEnv idx = some (.basic .u64))
    (hnotIn : notIn envL.siteEnv a)
    (houtbound : isBor = .siteBorrowMut →
      check_outbound envL.pathEnv s (fun re => only_matches_empty (simplify re)))
    (hfresh : freshRefInEnv rf envL)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := insert (delete (delete envL.siteEnv src) idx) a (.ref T rf .siteBorrowImm)
                   pathEnv := update_with_extension rf s [.vecElem] envL.pathEnv}
        cont retTypes) :
    typecheck_stmt lenv env (.letBind a (.vecImmBorrow src idx) cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  have hlook_src_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_src
  simp only [applySubstMoveType] at hlook_src_env
  have hrf_fresh_pe : rf ∉ envL.pathEnv.refs := freshRefInEnv_implies_freshRef rf envL hfresh
  ...
  have hrf'_not_root := nextFreshRefInEnv_not_root env
  have hrf'_not_mapped : rf' ∉ envL.pathEnv.refs.map σ := fun h => hrf'_fresh_pe (hrefs.mem_iff.mp h)
  have hs_mem := hsite_tracked _ _ _ _ hlook_src
  apply typecheck_stmt.let_bind_vecImmBorrow lenv env a src idx _ (σ s) rf' isBor _ _
    hlook_src_env
    (SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_idx |>.trans
      (by simp only [applySubstMoveType]))
    (SiteEnvSubstEquiv_notIn σ _ _ _ hse hnotIn)
    (fun h => check_outbound_weaken σ _ _ s hrefs hpaths hs_mem (houtbound h))
    hrf'_fresh_prop
  apply ih
  · -- subsumes with extendSubst σ rf rf'
    refine ⟨extendSubst σ rf rf',
      ...
      VarEnvSubstEquiv_extend σ rf rf' _ _ hve
        (fun x bt s' bk ms hlook' =>
          Ne.symm (freshRefInEnv_ne_varEnv_ref rf envL x .validVar bt s' bk ms hfresh hlook')),
      SiteEnvSubstEquiv_extend_insert_ref σ rf rf' _ _ a _ _
        (SiteEnvSubstEquiv_delete σ _ _ idx (SiteEnvSubstEquiv_delete σ _ _ src hse))
        (fun k bt r bk hlook' => by
          -- hlook' : lookup (delete (delete se src) idx) k = some (.ref bt r bk)
          -- outermost delete is idx, peel it first
          by_cases hkidx : k = idx
          · subst hkidx; rw [lookup_delete_same] at hlook'; cases hlook'
          · rw [lookup_delete_ne _ _ _ hkidx] at hlook'
            by_cases hksrc : k = src
            · subst hksrc; rw [lookup_delete_same] at hlook'; cases hlook'
            · rw [lookup_delete_ne _ _ _ hksrc] at hlook'
              exact Ne.symm (freshRefInEnv_ne_siteEnv_ref rf envL k bt r bk hfresh hlook')),
      ?_, ?_, ?_, ?_⟩
    · simp only [uwe_refs_fresh rf s _ envL.pathEnv hrf_fresh_pe,
          uwe_refs_fresh rf' (σ s) _ env.pathEnv hrf'_fresh_pe]
    ...
    · exact extendSubst_injective σ rf rf' _ hinj hrf_fresh_pe hrf'_not_mapped
    · exact extendSubst_nonroot σ rf rf' hnonroot hrf'_not_root
    · exact ⟨path_inclusion_update_with_extension σ _ _ rf rf' s [.vecElem]
        hpaths hrf_fresh_pe hrf'_not_mapped hs_mem, henum_equiv⟩
  · exact hfuneq
  · exact ⟨update_with_extension_wellformed rf s [.vecElem] envL.pathEnv
            hwfL.pathEnv_wf hrf_ne_root,
           SiteEnv.insert_refs_not_root _ _ _
            (SiteEnv.delete_refs_not_root _ _ (SiteEnv.delete_refs_not_root _ _ hwfL.siteEnv_wf)) hrf_ne_root,
           hwfL.varEnv_wf⟩
  · exact ⟨update_with_extension_wellformed rf' (σ s) [.vecElem] env.pathEnv
            hwfE.pathEnv_wf hrf'_not_root,
           SiteEnv.insert_refs_not_root _ _ _
            (SiteEnv.delete_refs_not_root _ _ (SiteEnv.delete_refs_not_root _ _ hwfE.siteEnv_wf)) hrf'_not_root,
           hwfE.varEnv_wf⟩
  · -- site_tracked
    -- siteEnv is insert (delete (delete envL.siteEnv src) idx) a (.ref T rf .siteBorrowImm)
    -- so after insert, inner structure is (delete (delete se src) idx): peel idx first
    intro s' bt_s r bk hlook_s'
    simp only [uwe_refs_fresh rf s _ envL.pathEnv hrf_fresh_pe]
    by_cases hs'_a : s' = a
    · subst hs'_a; rw [lookup_insert_same] at hlook_s'
      simp only [Option.some.injEq, MoveType.ref.injEq] at hlook_s'
      obtain ⟨-, rfl, -⟩ := hlook_s'
      exact .head _
    · rw [lookup_insert_ne _ _ _ _ hs'_a] at hlook_s'
      -- hlook_s' : lookup (delete (delete envL.siteEnv src) idx) s' = some (.ref ...)
      -- outermost delete is idx
      have hlook_orig : lookup envL.siteEnv s' = some (.ref bt_s r bk) := by
        by_cases hs'idx : s' = idx
        · subst hs'idx; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
        · rw [lookup_delete_ne _ _ _ hs'idx] at hlook_s'
          by_cases hs'src : s' = src
          · subst hs'src; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
          · rwa [lookup_delete_ne _ _ _ hs'src] at hlook_s'
      exact .tail _ (hsite_tracked _ _ _ _ hlook_orig)
  · -- var_tracked
    intro x bt r bk ms hlook_x
    simp only [uwe_refs_fresh rf s _ envL.pathEnv hrf_fresh_pe]
    exact .tail _ (hvar_tracked _ _ _ _ _ hlook_x)
  · -- RefsUnique: insert into (delete (delete se src) idx) with fresh rf
    -- peel idx first (outermost), then src
    exact RefsUnique_insert_fresh_ref _ _ a _ rf _ (RefsUnique_delete_site _ _ _ (RefsUnique_delete_site _ _ _ huniq))
      (fun x bt r bk' ms hlook' =>
        Ne.symm (freshRefInEnv_ne_varEnv_ref rf envL x .validVar bt r bk' ms hfresh hlook'))
      (fun s' bt r bk' hlook' => by
        by_cases hs'idx : s' = idx
        · subst hs'idx; rw [lookup_delete_same] at hlook'; cases hlook'
        · rw [lookup_delete_ne _ _ _ hs'idx] at hlook'
          by_cases hs'src : s' = src
          · subst hs'src; rw [lookup_delete_same] at hlook'; cases hlook'
          · rw [lookup_delete_ne _ _ _ hs'src] at hlook'
            exact Ne.symm (freshRefInEnv_ne_siteEnv_ref rf envL s' bt r bk' hfresh hlook'))
  · exact update_with_extension_paths_to_non_member _ _ _ _ hpaths_to_nm
      (Or.inl (hrefs.mem_iff.mp (List.mem_map_of_mem hs_mem)))
  · exact update_with_extension_paths_from_non_member _ _ _ _ hpaths_from_nm
      (Or.inl (hrefs.mem_iff.mp (List.mem_map_of_mem hs_mem)))
...
private theorem weaken_let_bind_vecMutBorrow
    (lenv : LabelEnv) (envL env : TypeEnv)
    (a src idx : Site) (T : BasicMoveType) (s rf : Aref)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_src : lookup envL.siteEnv src = some (.ref (.tvec T) s .siteBorrowMut))
    (hlook_idx : lookup envL.siteEnv idx = some (.basic .u64))
    (hnotIn : notIn envL.siteEnv a)
    (houtbound : check_outbound envL.pathEnv s (fun re => only_matches_empty (simplify re)))
    (hfresh : freshRefInEnv rf envL)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := insert (delete (delete envL.siteEnv src) idx) a (.ref T rf .siteBorrowMut)
                   pathEnv := update_with_extension rf s [.vecElem] envL.pathEnv}
        cont retTypes) :
    typecheck_stmt lenv env (.letBind a (.vecMutBorrow src idx) cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  have hlook_src_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_src
  simp only [applySubstMoveType] at hlook_src_env
  have hrf_fresh_pe : rf ∉ envL.pathEnv.refs := freshRefInEnv_implies_freshRef rf envL hfresh
  ...
  have hrf'_not_mapped : rf' ∉ envL.pathEnv.refs.map σ := fun h => hrf'_fresh_pe (hrefs.mem_iff.mp h)
  have hs_mem := hsite_tracked _ _ _ _ hlook_src
  apply typecheck_stmt.let_bind_vecMutBorrow lenv env a src idx _ (σ s) rf' _ _
    hlook_src_env
    (SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_idx |>.trans
      (by simp only [applySubstMoveType]))
    (SiteEnvSubstEquiv_notIn σ _ _ _ hse hnotIn)
    (check_outbound_weaken σ _ _ s hrefs hpaths hs_mem houtbound)
    hrf'_fresh_prop
  apply ih
  · -- subsumes with extendSubst σ rf rf'
    refine ⟨extendSubst σ rf rf',
  ...
  · -- var_tracked
    ...
    simp only [uwe_refs_fresh rf s _ envL.pathEnv hrf_fresh_pe]
    exact .tail _ (hvar_tracked _ _ _ _ _ hlook_x)
  · -- RefsUnique: insert into (delete src (delete idx se)) with fresh rf
    exact RefsUnique_insert_fresh_ref _ _ a _ rf _ (RefsUnique_delete_site _ _ _ (RefsUnique_delete_site _ _ _ huniq))
      (fun x bt r bk' ms hlook' =>
        Ne.symm (freshRefInEnv_ne_varEnv_ref rf envL x .validVar bt r bk' ms hfresh hlook'))
      (fun s' bt r bk' hlook' => by
...
private theorem weaken_let_bind_vecPopBack
    (lenv : LabelEnv) (envL env : TypeEnv)
    (a src : Site) (T : BasicMoveType) (r : Aref)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_src : lookup envL.siteEnv src = some (.ref (.tvec T) r .siteBorrowMut))
    (houtbound : check_outbound envL.pathEnv r (fun re => only_matches_empty (simplify re)))
    (hnotIn : notIn envL.siteEnv a)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := insert (delete envL.siteEnv src) a (.basic T)
                   pathEnv := garbage_collect envL.pathEnv r}
        cont retTypes) :
    typecheck_stmt lenv env (.letBind a (.vecPopBack src) cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  have hlook_src_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_src
  simp only [applySubstMoveType] at hlook_src_env
  have hr_mem := hsite_tracked _ _ _ _ hlook_src
  have hr_ne_root := hwfL.siteEnv_wf _ _ hlook_src
  have hσr_ne_root : σ r ≠ .root := hnonroot r hr_ne_root
  apply typecheck_stmt.let_bind_vecPopBack lenv env _ _ _ (σ r) _ _
    hlook_src_env
    (check_outbound_weaken σ _ _ r hrefs hpaths hr_mem houtbound)
    (SiteEnvSubstEquiv_notIn σ _ _ _ hse hnotIn)
  apply ih
  · -- subsumes: garbage_collect
    refine ⟨σ, hid, hve,
      SiteEnvSubstEquiv_insert σ _ _ _ _ _
        (SiteEnvSubstEquiv_delete σ _ _ _ hse) (applySubstMoveType_basic σ _),
      ?_, ?_, hnonroot, ?_⟩
  ...
  · exact ⟨garbage_collect_wellformed _ _ hwfL.pathEnv_wf hr_ne_root,
           SiteEnv.insert_refs_not_root _ _ _
            (SiteEnv.delete_refs_not_root _ _ hwfL.siteEnv_wf) trivial,
           hwfL.varEnv_wf⟩
  · exact ⟨garbage_collect_wellformed _ _ hwfE.pathEnv_wf hσr_ne_root,
           SiteEnv.insert_refs_not_root _ _ _
            (SiteEnv.delete_refs_not_root _ _ hwfE.siteEnv_wf) trivial,
           hwfE.varEnv_wf⟩
  · -- site_tracked
    intro s' bt' r' bk' hlook_s'
    simp only [garbage_collect, List.mem_filter, decide_eq_true_eq]
...
private theorem weaken_vecPushBack_rule
    (lenv : LabelEnv) (envL env : TypeEnv)
    (refSite val : Site) (T : BasicMoveType) (r : Aref)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_ref : lookup envL.siteEnv refSite = some (.ref (.tvec T) r .siteBorrowMut))
    (hlook_val : lookup envL.siteEnv val = some (.basic T))
    (houtbound : check_outbound envL.pathEnv r (fun re => only_matches_empty (simplify re)))
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := delete (delete envL.siteEnv val) refSite
                   pathEnv := garbage_collect envL.pathEnv r}
        cont retTypes) :
    typecheck_stmt lenv env (.vecPushBack refSite val cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  have hlook_ref_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_ref
  have hlook_val_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_val
  simp only [applySubstMoveType] at hlook_ref_env hlook_val_env
  have hr_mem := hsite_tracked _ _ _ _ hlook_ref
  have hr_ne_root := hwfL.siteEnv_wf _ _ hlook_ref
  have hσr_ne_root : σ r ≠ .root := hnonroot r hr_ne_root
  apply typecheck_stmt.vecPushBack_rule lenv env _ _ _ (σ r) _ _
    hlook_ref_env hlook_val_env
    (check_outbound_weaken σ _ _ r hrefs hpaths hr_mem houtbound)
  apply ih
  · -- subsumes: garbage_collect
    refine ⟨σ, hid, hve,
  ...
  · -- site_tracked
    intro s' bt' r' bk' hlook_s'
    simp only [garbage_collect, List.mem_filter, decide_eq_true_eq]
    by_cases hs'ref : s' = refSite
    · subst hs'ref; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
    · rw [lookup_delete_ne _ _ _ hs'ref] at hlook_s'
      by_cases hs'val : s' = val
      · subst hs'val; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
      · rw [lookup_delete_ne _ _ _ hs'val] at hlook_s'
        exact ⟨hsite_tracked _ _ _ _ hlook_s',
          fun hr'_eq => by rw [hr'_eq] at hlook_s'; exact (huniq r).2.1 s' refSite _ _ _ _ hs'ref hlook_s' hlook_ref⟩
  · -- var_tracked
    intro x' bt' r' bk' ms' hlook_x'
    simp only [garbage_collect, List.mem_filter, decide_eq_true_eq]
    exact ⟨hvar_tracked _ _ _ _ _ hlook_x',
      fun hr'_eq => by rw [hr'_eq] at hlook_x'; exact (huniq r).1 x' _ _ _ refSite _ _ hlook_x' hlook_ref⟩
  · exact RefsUnique_delete_site _ _ _ (RefsUnique_delete_site _ _ _ huniq)
  · exact garbage_collect_paths_to_non_member _ _ hpaths_to_nm
  · exact garbage_collect_paths_from_non_member _ _ hpaths_from_nm
  · exact self_loop_only_empty_gc env.pathEnv _ hself_loop_only_empty
...
private theorem weaken_vecSwap_rule
    (lenv : LabelEnv) (envL env : TypeEnv)
    (refSite idx1 idx2 : Site) (T : BasicMoveType) (r : Aref)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_ref : lookup envL.siteEnv refSite = some (.ref (.tvec T) r .siteBorrowMut))
    (hlook_idx1 : lookup envL.siteEnv idx1 = some (.basic .u64))
    (hlook_idx2 : lookup envL.siteEnv idx2 = some (.basic .u64))
    (houtbound : check_outbound envL.pathEnv r (fun re => only_matches_empty (simplify re)))
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := delete (delete (delete envL.siteEnv idx2) idx1) refSite
                   pathEnv := garbage_collect envL.pathEnv r}
        cont retTypes) :
    typecheck_stmt lenv env (.vecSwap refSite idx1 idx2 cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
  have hlook_ref_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_ref
  simp only [applySubstMoveType] at hlook_ref_env
  have hr_mem := hsite_tracked _ _ _ _ hlook_ref
  have hr_ne_root := hwfL.siteEnv_wf _ _ hlook_ref
  have hσr_ne_root : σ r ≠ .root := hnonroot r hr_ne_root
  apply typecheck_stmt.vecSwap_rule lenv env _ _ _ _ (σ r) _ _
    hlook_ref_env
    (SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_idx1 |>.trans
      (by simp only [applySubstMoveType]))
    (SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_idx2 |>.trans
      (by simp only [applySubstMoveType]))
    (check_outbound_weaken σ _ _ r hrefs hpaths hr_mem houtbound)
  apply ih
  · -- subsumes: garbage_collect
    refine ⟨σ, hid, hve,
      SiteEnvSubstEquiv_delete σ _ _ _ (SiteEnvSubstEquiv_delete σ _ _ _
        (SiteEnvSubstEquiv_delete σ _ _ _ hse)),
      ?_, ?_, hnonroot, ?_⟩
    · simp only [garbage_collect]
      rw [map_filter_ne σ _ r hinj hr_mem]
  ...
  · exact hfuneq
  · exact ⟨garbage_collect_wellformed _ _ hwfL.pathEnv_wf hr_ne_root,
           SiteEnv.delete_refs_not_root _ _ (SiteEnv.delete_refs_not_root _ _
            (SiteEnv.delete_refs_not_root _ _ hwfL.siteEnv_wf)),
           hwfL.varEnv_wf⟩
  · exact ⟨garbage_collect_wellformed _ _ hwfE.pathEnv_wf hσr_ne_root,
           SiteEnv.delete_refs_not_root _ _ (SiteEnv.delete_refs_not_root _ _
  ...
  · -- site_tracked
    ...
    · subst hs'ref; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
    · rw [lookup_delete_ne _ _ _ hs'ref] at hlook_s'
      by_cases hs'idx1 : s' = idx1
      · subst hs'idx1; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
      · rw [lookup_delete_ne _ _ _ hs'idx1] at hlook_s'
        by_cases hs'idx2 : s' = idx2
        · subst hs'idx2; rw [lookup_delete_same] at hlook_s'; cases hlook_s'
        · rw [lookup_delete_ne _ _ _ hs'idx2] at hlook_s'
          exact ⟨hsite_tracked _ _ _ _ hlook_s',
            fun hr'_eq => by rw [hr'_eq] at hlook_s'; exact (huniq r).2.1 s' refSite _ _ _ _ hs'ref hlook_s' hlook_ref⟩
  · -- var_tracked
    intro x' bt' r' bk' ms' hlook_x'
    ...
    exact ⟨hvar_tracked _ _ _ _ _ hlook_x',
      fun hr'_eq => by rw [hr'_eq] at hlook_x'; exact (huniq r).1 x' _ _ _ refSite _ _ hlook_x' hlook_ref⟩
  · exact RefsUnique_delete_site _ _ _ (RefsUnique_delete_site _ _ _ (RefsUnique_delete_site _ _ _ huniq))
  · exact garbage_collect_paths_to_non_member _ _ hpaths_to_nm
  · exact garbage_collect_paths_from_non_member _ _ hpaths_from_nm
  · exact self_loop_only_empty_gc env.pathEnv _ hself_loop_only_empty
  · simp only [garbage_collect, List.mem_filter, decide_eq_true_eq]
...
/-- populate_call_outputs preserves enumEnv -/
lemma populate_call_outputs_enumEnv (env env' : TypeEnv) (as : List Site) (rets : List ParamType)
    (outRefs : List Aref)
    (hpop : populate_call_outputs env as rets outRefs = some env') :
    env'.enumEnv = env.enumEnv := by
  induction as generalizing env env' rets outRefs with
  | nil =>
    cases rets with
    | nil => simp only [populate_call_outputs] at hpop; split at hpop <;> simp_all
...
private lemma populate_call_outputs_subsumes
    ...
    (henum_equiv : LookupEquiv envL.enumEnv envE.enumEnv)
    ...
    | nil =>
      ...
      · split at hpopE
        · simp only [Option.some.injEq] at hpopL hpopE; subst hpopL; subst hpopE
          exact ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩
        · simp at hpopE
      · simp at hpopL
    ...
    | cons pt rets' =>
      ...
      | none =>
        ...
        exact ih_as
          ...
          hnotRootL hnotRootE hpaths_to_nm hpaths_from_nm hfuneq henum_equiv hpopL hpopE
      | some b =>
        cases b <;> (
          cases outRefsL with
          | nil => simp [populate_call_outputs] at hpopL
            ...
            | cons rE outRefsE' =>
              ...
              exact ih_as
                ...
                (update_with_extension_paths_from_non_member rE rE [] envE.pathEnv
                  hpaths_from_nm (Or.inr rfl))
                hfuneq henum_equiv hpopL hpopE)
...
private lemma call_connect_subsumes
  ...
  refine ⟨σ, hid, ?_, ?_, ?_, ?_, hnonroot, ?_, ?_⟩
  ...
  · -- LookupEquiv: call_connect preserves enumEnv
    show LookupEquiv (call_connect_inputs_outputs envL as bs).enumEnv
                     (call_connect_inputs_outputs envE as bs).enumEnv
    simp only [call_connect_inputs_outputs]; exact henum_equiv
...
lemma call_connect_subsumes_self (env' : TypeEnv) (results args : List Site)
  ...
  refine ⟨id, fun _ _ => rfl, ?_, ?_, ?_, fun _ _ _ _ h => h, fun _ h => h, ?_, fun _ => rfl⟩
  · -- VarEnvSubstEquiv id
...
private theorem weaken_call
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
      (let env'' := call_connect_inputs_outputs envL' as bs
       {env'' with siteEnv := AssocMap.deleteAll env''.siteEnv bs})
      cont retTypes) :
    typecheck_stmt lenv env (.call as fnName bs cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths_incl, henum_equiv⟩ := hsub
  ...
  -- WF for call_connect environments
  have hwfL_cc := call_connect_inputs_outputs_wf envL' as bs hwfL'
  have hwfE_cc := call_connect_inputs_outputs_wf envE' as bs hwfE'
  -- WF for deleteAll-wrapped environments
  have hwfL_del : (let env'' := call_connect_inputs_outputs envL' as bs
      {env'' with siteEnv := AssocMap.deleteAll env''.siteEnv bs}).WellFormed :=
    ⟨hwfL_cc.pathEnv_wf, SiteEnv.deleteAll_refs_not_root _ _ hwfL_cc.siteEnv_wf, hwfL_cc.varEnv_wf⟩
  have hwfE_del : (let env'' := call_connect_inputs_outputs envE' as bs
      {env'' with siteEnv := AssocMap.deleteAll env''.siteEnv bs}).WellFormed :=
    ⟨hwfE_cc.pathEnv_wf, SiteEnv.deleteAll_refs_not_root _ _ hwfE_cc.siteEnv_wf, hwfE_cc.varEnv_wf⟩
  ...
  have hst_E : ∀ s bt r bk, lookup env.siteEnv s = some (.ref bt r bk) → r ∈ env.pathEnv.refs := by
    ...
    | ref bt' r' bk' =>
      ...
      exact hrefs.symm.mem_iff.mpr (List.mem_map.mpr ⟨r', hsite_tracked s bt' r' bk' hlook1, rfl⟩)
  ...
  -- Subsumption between call_connect environments
  have hsub_cc := call_connect_subsumes envL' envE' as bs
    (populate_call_outputs_subsumes envL env envL' envE' as rets outRefsL outRefsE σ
      ...
      hpaths_to_nm hpaths_from_nm hfuneq henum_equiv hpopL hpopE)
    hst_L' hst_E'
  -- Precompute properties for call_connect envs (avoiding let-binding issues with rw)
  have hst_cc_L : ∀ s bt r bk, lookup (call_connect_inputs_outputs envL' as bs).siteEnv s =
      some (.ref bt r bk) → r ∈ (call_connect_inputs_outputs envL' as bs).pathEnv.refs := by
    intro s bt r bk hlook
    rw [call_connect_siteEnv] at hlook
    exact call_connect_refs_mono envL' as bs r (hst_L' s bt r bk hlook)
  have hvt_cc_L : ∀ x bt r bk ms, lookup (call_connect_inputs_outputs envL' as bs).varEnv x =
      some (.validVar, .ref bt r bk, ms) → r ∈ (call_connect_inputs_outputs envL' as bs).pathEnv.refs := by
    intro x bt r bk ms hlook
    rw [call_connect_varEnv] at hlook
    exact call_connect_refs_mono envL' as bs r (hvt_L' x bt r bk ms hlook)
  have huniq_cc_L : RefsUnique (call_connect_inputs_outputs envL' as bs).varEnv
      (call_connect_inputs_outputs envL' as bs).siteEnv := by
  ...
  exact typecheck_stmt.call lenv env fnName as bs params rets outRefsE envE' cont retTypes
    ...
    (ih (let env'' := call_connect_inputs_outputs envE' as bs
      ...
      (by obtain ⟨σ', hid', hve', hse', hrefs', hinj', hnonroot', hpaths'⟩ := hsub_cc
          exact ⟨σ', hid', hve', SiteEnvSubstEquiv_deleteAll σ' _ _ bs hse',
                 hrefs', hinj', hnonroot', hpaths'⟩)
      -- funEnv equality
      (by simp only [call_connect_funEnv,
            populate_call_outputs_funEnv _ _ _ _ _ hpopL,
            populate_call_outputs_funEnv _ _ _ _ _ hpopE, hfuneq])
      -- WellFormed for envL-side (deleteAll-wrapped)
      hwfL_del
      -- WellFormed for envE-side (deleteAll-wrapped)
      hwfE_del
      -- site_tracked for envL-side (deleteAll-wrapped)
      (site_tracked_deleteAll _ _ bs hst_cc_L)
      -- var_tracked for envL-side (deleteAll-wrapped): varEnv unchanged by deleteAll
      hvt_cc_L
      -- RefsUnique for envL-side (deleteAll-wrapped)
      (RefsUnique_deleteAll _ _ _ huniq_cc_L)
      -- paths_to_nm for envE-side (deleteAll-wrapped): pathEnv unchanged by deleteAll
      (call_connect_paths_to_nm envE' as bs hto_E' hst_E')
      -- paths_from_nm for envE-side (deleteAll-wrapped)
      (call_connect_paths_from_nm envE' as bs hfrom_E' hst_E')
      -- self_loop_only_empty for envE-side (deleteAll-wrapped)
      (call_connect_self_loop_only_empty envE' as bs hsl_E')
      -- root ∈ pathEnv.refs for envL-side
...
private lemma weaken_ret (lenv : LabelEnv) (envL env : TypeEnv) (as : List Site)
    ...
    (hwritable : ∀ a ∈ as, ∀ bt r, lookup envL.siteEnv a = some (.ref bt r .siteBorrowMut) →
      ∀ b, b ∉ as →
        ∀ bt' r' bk', lookup envL.siteEnv b = some (.ref bt' r' bk') →
          ∀ p, interpret_regex (envL.pathEnv.paths (r, r')) p → p = [])
  ...
  obtain ⟨σ, hid_σ, _, hse, hrefs, hinj, _, hpaths_sub, _⟩ := hsub
  ...
  exact typecheck_stmt.ret lenv env _ _
    ...
    (by -- writability: for non-returned live ref sites
      intro a ha_in bt r hlook_env b hb bt' r' bk' hlook_b_env p hp_env
      obtain ⟨τ1, hlook1, hsubst1⟩ :=
        SiteEnvSubstEquiv_lookup_some_inv σ _ _ a _ hse hlook_env
      obtain ⟨τ2, hlook2, hsubst2⟩ :=
        SiteEnvSubstEquiv_lookup_some_inv σ _ _ b _ hse hlook_b_env
      cases τ1 with
      | basic _ => simp [applySubstMoveType] at hsubst1
      | ref bt1 r1 bk1 =>
        simp only [applySubstMoveType, MoveType.ref.injEq] at hsubst1
        have hrr1 : σ r1 = r := hsubst1.2.1
        have hbk1 : bk1 = .siteBorrowMut := hsubst1.2.2
        rw [hbk1] at hlook1
        have hr1_mem := hsite_tracked a bt1 r1 .siteBorrowMut hlook1
        cases τ2 with
        | basic _ => simp [applySubstMoveType] at hsubst2
        | ref bt2 r2 bk2 =>
          simp only [applySubstMoveType, MoveType.ref.injEq] at hsubst2
          have hrr2 : σ r2 = r' := hsubst2.2.1
          have hr2_mem := hsite_tracked b bt2 r2 bk2 hlook2
          rw [← hrr1, ← hrr2] at hp_env
          by_cases heq : r1 = r2
          · subst heq; exact hself_loop_only_empty _ p hp_env
          · exact hwritable a ha_in bt1 r1 hlook1 b hb bt2 r2 bk2 hlook2 p
              (hpaths_sub r1 r2 hr1_mem hr2_mem p hp_env))
    (by -- no aliasing
...
-- ============================================================
-- Helper lemmas for addRefFieldSites weakening
-- ============================================================

/-- addRefFieldSites doesn't change varEnv -/
theorem addRefFieldSites_varEnv_eq (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv) :
    (addRefFieldSites r bk fentries qualify fields env).varEnv = env.varEnv := by
...
/-- addRefFieldSites doesn't change funEnv -/
theorem addRefFieldSites_funEnv_eq (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv) :
    (addRefFieldSites r bk fentries qualify fields env).funEnv = env.funEnv := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => rfl
  | cons hd rest ih =>
...
theorem addRefFieldSites_enumEnv_eq (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv) :
    (addRefFieldSites r bk fentries qualify fields env).enumEnv = env.enumEnv := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => rfl
  | cons hd rest ih =>
...
/-- All refs in env.pathEnv.refs remain in (addRefFieldSites r bk fentries fields env).pathEnv.refs -/
theorem addRefFieldSites_refs_subset (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (u : Aref) (hu : u ∈ env.pathEnv.refs) :
    u ∈ (addRefFieldSites r bk fentries qualify fields env).pathEnv.refs := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact hu
  | cons hd rest ih =>
    simp only [List.foldl]
    cases hlook : lookup fentries hd.1 with
    | none => exact ih env hu
    | some bt =>
      apply ih
      have hfresh := nextFreshRefInEnv_not_in_pathEnv env
      rw [uwe_refs_fresh _ _ _ _ hfresh]
      exact .tail _ hu

/-- PathEnv.WellFormed preserved by addRefFieldSites -/
theorem addRefFieldSites_pathEnv_wf (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hwf : PathEnv.WellFormed env.pathEnv) :
    PathEnv.WellFormed (addRefFieldSites r bk fentries qualify fields env).pathEnv := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact hwf
  | cons hd rest ih =>
    simp only [List.foldl]
    cases hlook : lookup fentries hd.1 with
    | none => exact ih env hwf
    | some bt =>
      apply ih
      exact update_with_extension_wellformed _ _ _ _ hwf (nextFreshRefInEnv_not_root env)

/-- SiteEnv.RefsNotRoot preserved by addRefFieldSites -/
theorem addRefFieldSites_siteEnv_wf (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hwf : SiteEnv.RefsNotRoot env.siteEnv) :
    SiteEnv.RefsNotRoot (addRefFieldSites r bk fentries qualify fields env).siteEnv := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact hwf
  | cons hd rest ih =>
    ...
    | some bt =>
      apply ih
      exact SiteEnv.insert_refs_not_root _ _ _ hwf (nextFreshRefInEnv_not_root env)

/-- site_tracked: refs in siteEnv entries are tracked in pathEnv.refs after addRefFieldSites -/
theorem addRefFieldSites_site_tracked (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hst : ∀ s bt' r' bk', lookup env.siteEnv s = some (.ref bt' r' bk') → r' ∈ env.pathEnv.refs)
    (hr_mem : r ∈ env.pathEnv.refs) :
    ∀ s bt' r' bk', lookup (addRefFieldSites r bk fentries qualify fields env).siteEnv s = some (.ref bt' r' bk') →
      r' ∈ (addRefFieldSites r bk fentries qualify fields env).pathEnv.refs := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact hst
  | cons hd rest ih =>
    ...
    | some bt =>
      ...
      · intro s bt' r' bk' hlook_s
        rw [uwe_refs_fresh _ _ _ _ hfresh]
        by_cases hsa : s = hd.2
        · subst hsa; rw [lookup_insert_same] at hlook_s
          simp only [Option.some.injEq, MoveType.ref.injEq] at hlook_s
          obtain ⟨-, rfl, -⟩ := hlook_s
          exact .head _
        · rw [lookup_insert_ne _ _ _ _ hsa] at hlook_s
          exact .tail _ (hst _ _ _ _ hlook_s)
      · rw [uwe_refs_fresh _ _ _ _ hfresh]
        exact .tail _ hr_mem

/-- var_tracked: refs in varEnv entries are tracked after addRefFieldSites (varEnv unchanged, refs grow) -/
theorem addRefFieldSites_var_tracked (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hvt : ∀ x bt' r' bk' ms, lookup env.varEnv x = some (.validVar, .ref bt' r' bk', ms) → r' ∈ env.pathEnv.refs) :
    ∀ x bt' r' bk' ms, lookup (addRefFieldSites r bk fentries qualify fields env).varEnv x = some (.validVar, .ref bt' r' bk', ms) →
      r' ∈ (addRefFieldSites r bk fentries qualify fields env).pathEnv.refs := by
  intro x bt' r' bk' ms hlook
  rw [addRefFieldSites_varEnv_eq] at hlook
  exact addRefFieldSites_refs_subset r bk fentries qualify fields env _ (hvt x bt' r' bk' ms hlook)

/-- RefsUnique preserved by addRefFieldSites -/
theorem addRefFieldSites_RefsUnique (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (huniq : RefsUnique env.varEnv env.siteEnv) :
    RefsUnique (addRefFieldSites r bk fentries qualify fields env).varEnv
              (addRefFieldSites r bk fentries qualify fields env).siteEnv := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact huniq
  | cons hd rest ih =>
    simp only [List.foldl]
    cases hlook : lookup fentries hd.1 with
    | none => exact ih env huniq
    | some bt =>
      have hfresh := nextFreshRefInEnv_fresh_prop env
      exact ih _ (RefsUnique_insert_fresh_ref _ _ _ _ _ _ huniq
        (fun x bt' s' bk' ms hlook' =>
          Ne.symm (freshRefInEnv_ne_varEnv_ref _ _ x .validVar bt' s' bk' ms hfresh hlook'))
        (fun s bt' r' bk' hlook' =>
          Ne.symm (freshRefInEnv_ne_siteEnv_ref _ _ s bt' r' bk' hfresh hlook')))

/-- paths_to_non_member preserved by addRefFieldSites -/
theorem addRefFieldSites_paths_to_nm (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hpaths_to_nm : ∀ u v p, v ∉ env.pathEnv.refs → v ≠ .root → u ≠ v →
      ¬interpret_regex (env.pathEnv.paths (u, v)) p)
    (hr_mem : r ∈ env.pathEnv.refs) :
    ∀ u v p, v ∉ (addRefFieldSites r bk fentries qualify fields env).pathEnv.refs → v ≠ .root → u ≠ v →
      ¬interpret_regex ((addRefFieldSites r bk fentries qualify fields env).pathEnv.paths (u, v)) p := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact hpaths_to_nm
  | cons hd rest ih =>
    simp only [List.foldl]
    cases hlook : lookup fentries hd.1 with
    | none => exact ih env hpaths_to_nm hr_mem
    | some bt =>
      have hfresh := nextFreshRefInEnv_not_in_pathEnv env
      apply ih
      · exact update_with_extension_paths_to_non_member _ _ _ _ hpaths_to_nm
          (Or.inl hr_mem)
      · rw [uwe_refs_fresh _ _ _ _ hfresh]; exact .tail _ hr_mem

/-- paths_from_non_member preserved by addRefFieldSites -/
theorem addRefFieldSites_paths_from_nm (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hpaths_from_nm : ∀ u v p, u ∉ env.pathEnv.refs → u ≠ .root → u ≠ v →
      ¬interpret_regex (env.pathEnv.paths (u, v)) p)
    (hr_mem : r ∈ env.pathEnv.refs) :
    ∀ u v p, u ∉ (addRefFieldSites r bk fentries qualify fields env).pathEnv.refs → u ≠ .root → u ≠ v →
      ¬interpret_regex ((addRefFieldSites r bk fentries qualify fields env).pathEnv.paths (u, v)) p := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact hpaths_from_nm
  | cons hd rest ih =>
    simp only [List.foldl]
    cases hlook : lookup fentries hd.1 with
    | none => exact ih env hpaths_from_nm hr_mem
    | some bt =>
      have hfresh := nextFreshRefInEnv_not_in_pathEnv env
      apply ih
      · exact update_with_extension_paths_from_non_member _ _ _ _ hpaths_from_nm
...
/-- self_loop_only_empty preserved by addRefFieldSites -/
theorem addRefFieldSites_self_loop_only_empty (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hself : ∀ u p, interpret_regex (env.pathEnv.paths (u, u)) p → p = []) :
    ∀ u p, interpret_regex ((addRefFieldSites r bk fentries qualify fields env).pathEnv.paths (u, u)) p → p = [] := by
  unfold addRefFieldSites
  induction fields generalizing env with
  | nil => exact hself
  | cons hd rest ih =>
    simp only [List.foldl]
    cases hlook : lookup fentries hd.1 with
    | none => exact ih env hself
    | some bt =>
      apply ih
      exact self_loop_only_empty_uwe _ _ _ _ hself

/-- root stays in refs after addRefFieldSites -/
theorem addRefFieldSites_root_in_refs (r : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site)) (env : TypeEnv)
    (hroot : Aref.root ∈ env.pathEnv.refs) :
    Aref.root ∈ (addRefFieldSites r bk fentries qualify fields env).pathEnv.refs :=
  addRefFieldSites_refs_subset r bk fentries qualify fields env _ hroot

/-- Main subsumption lemma: addRefFieldSites preserves subsumption with extended σ.
    Given subsumption between envL and env via σ, and rL tracked in envL with σ rL = rE,
    addRefFieldSites rL bk fentries fields envL subsumes
    addRefFieldSites rE bk fentries fields env via some σ'. -/
private theorem addRefFieldSites_subsumes
    (σ : Aref → Aref) (rL rE : Aref) (bk : BorrowingKind)
    (fentries : AssocMap Field BasicMoveType) (qualify : Field → Field) (fields : List (Field × Site))
    (envL env : TypeEnv)
    (hid : ∀ r, (∀ n, r ≠ .refid n) → σ r = r)
    (hve : VarEnvSubstEquiv σ envL.varEnv env.varEnv)
    (hse : SiteEnvSubstEquiv σ envL.siteEnv env.siteEnv)
    (hrefs : (envL.pathEnv.refs.map σ).Perm env.pathEnv.refs)
    (hinj_σ : ∀ u v, u ∈ envL.pathEnv.refs → v ∈ envL.pathEnv.refs → σ u = σ v → u = v)
    (hnonroot : ∀ r, r ≠ .root → σ r ≠ .root)
    (hpaths_σ : ∀ u v, u ∈ envL.pathEnv.refs → v ∈ envL.pathEnv.refs →
      ∀ path, interpret_regex (env.pathEnv.paths (σ u, σ v)) path →
              interpret_regex (envL.pathEnv.paths (u, v)) path)
    (hσ_rL : σ rL = rE)
    (hrL_mem : rL ∈ envL.pathEnv.refs)
    (hsite_tracked : ∀ s bt r bk, lookup envL.siteEnv s = some (.ref bt r bk) → r ∈ envL.pathEnv.refs)
    (hvar_tracked : ∀ x bt r bk ms, lookup envL.varEnv x = some (.validVar, .ref bt r bk, ms) → r ∈ envL.pathEnv.refs)
    (henum_equiv : LookupEquiv envL.enumEnv env.enumEnv) :
    TypeEnv.subsumes (addRefFieldSites rL bk fentries qualify fields envL)
                     (addRefFieldSites rE bk fentries qualify fields env) := by
  unfold addRefFieldSites
  induction fields generalizing envL env σ with
  | nil => exact ⟨σ, hid, hve, hse, hrefs, hinj_σ, hnonroot, hpaths_σ, henum_equiv⟩
  | cons hd rest ih =>
    simp only [List.foldl]
    cases hlook : lookup fentries hd.1 with
    | none => exact ih σ envL env hid hve hse hrefs hinj_σ hnonroot hpaths_σ hσ_rL hrL_mem
                hsite_tracked hvar_tracked henum_equiv
    | some bt =>
      -- Both sides create fresh refs and extend
      have hrfL_fresh_pe := nextFreshRefInEnv_not_in_pathEnv envL
      have hrfL_fresh := nextFreshRefInEnv_fresh_prop envL
      have hrfL_ne_root := nextFreshRefInEnv_not_root envL
      have hrfL_refid : ∃ n, nextFreshRefInEnv envL = .refid n := by
        simp only [nextFreshRefInEnv]; exact ⟨_, rfl⟩
      have hrfE_fresh_pe := nextFreshRefInEnv_not_in_pathEnv env
      have hrfE_not_root := nextFreshRefInEnv_not_root env
      have hrfE_not_mapped : nextFreshRefInEnv env ∉ envL.pathEnv.refs.map σ :=
        fun h => hrfE_fresh_pe (hrefs.mem_iff.mp h)
      -- σ' rL = σ rL = rE because rL ≠ rfL (rL ∈ refs, rfL ∉ refs)
      have hrL_ne_rfL : rL ≠ nextFreshRefInEnv envL :=
        fun h => hrfL_fresh_pe (h ▸ hrL_mem)
      have hσ'_rL : extendSubst σ (nextFreshRefInEnv envL) (nextFreshRefInEnv env) rL = rE := by
        simp only [extendSubst, if_neg hrL_ne_rfL]; exact hσ_rL
      -- Apply IH with extended σ'
      -- Use dsimp to reduce `match some bt with | some bt => ... | none => ...`
      -- and struct field access like {pathEnv := X, ...}.pathEnv → X
      dsimp only
      apply ih (extendSubst σ (nextFreshRefInEnv envL) (nextFreshRefInEnv env))
      · exact extendSubst_id σ _ _ hid hrfL_refid
      · exact VarEnvSubstEquiv_extend σ _ _ _ _ hve
          (fun x bt' s' bk' ms hlook' =>
            Ne.symm (freshRefInEnv_ne_varEnv_ref _ envL x .validVar bt' s' bk' ms hrfL_fresh hlook'))
      · exact SiteEnvSubstEquiv_extend_insert_ref σ _ _ _ _ hd.2 bt bk
          hse
          (fun k bt' r' bk' hlook' =>
            Ne.symm (freshRefInEnv_ne_siteEnv_ref _ envL k bt' r' bk' hrfL_fresh hlook'))
      · -- refs perm
        simp only [uwe_refs_fresh _ rL _ envL.pathEnv hrfL_fresh_pe,
            uwe_refs_fresh _ rE _ env.pathEnv hrfE_fresh_pe]
        simp only [List.map, extendSubst, ite_true]
        rw [map_extendSubst_eq_map_σ σ _ _ _ hrfL_fresh_pe]
        exact List.Perm.cons _ hrefs
      · exact extendSubst_injective σ _ _ _ hinj_σ hrfL_fresh_pe hrfE_not_mapped
      · exact extendSubst_nonroot σ _ _ hnonroot hrfE_not_root
      · -- path inclusion: need to convert σ rL → rE
        intro u v hu hv path hinterp
        have h := path_inclusion_update_with_extension σ _ _ _ _ rL [.field (qualify hd.1)]
          hpaths_σ hrfL_fresh_pe hrfE_not_mapped hrL_mem u v hu hv path
        rw [hσ_rL] at h
        exact h hinterp
      · exact hσ'_rL
      · rw [uwe_refs_fresh _ rL _ envL.pathEnv hrfL_fresh_pe]; exact .tail _ hrL_mem
      · intro s bt' r' bk' hlook_s
        rw [uwe_refs_fresh _ rL _ envL.pathEnv hrfL_fresh_pe]
        by_cases hsa : s = hd.2
        · subst hsa; rw [lookup_insert_same] at hlook_s
          simp only [Option.some.injEq, MoveType.ref.injEq] at hlook_s
          obtain ⟨-, rfl, -⟩ := hlook_s
      ...
      · intro x bt' r' bk' ms hlook_x
        rw [uwe_refs_fresh _ rL _ envL.pathEnv hrfL_fresh_pe]
        exact .tail _ (hvar_tracked x bt' r' bk' ms hlook_x)
      · exact henum_equiv

-- Enum weakening lemmas

private theorem weaken_let_bind_packVariant
    (lenv : LabelEnv) (envL env : TypeEnv)
    (b : Site) (enumName variantName : Id) (fields : List (Field × Site))
    (enumDef : EnumDef) (variantDef : EnumVariantDef)
    (cont : Stmt) (retTypes : List ParamType)
    (hnotIn : notIn envL.siteEnv b)
    (hlookup_enum : AssocMap.lookup envL.enumEnv enumName = some enumDef)
    (hlookup_var : AssocMap.lookup enumDef.variants variantName = some variantDef)
    (hft : ∀ f a, (f, a) ∈ fields →
      ∃ bt, lookup envL.siteEnv a = some (.basic bt) ∧ lookup variantDef.fields f = some bt)
    (hfc : ∀ f, lookup variantDef.fields f ≠ none → ∃ a, (f, a) ∈ fields)
    (hfi : ∀ a₁ a₂, (∃ f₁ f₂, (f₁, a₁) ∈ fields ∧ (f₂, a₂) ∈ fields ∧ f₁ ≠ f₂) → a₁ ≠ a₂)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := insert (deleteAll envL.siteEnv (fields.map Prod.snd)) b
                                    (.basic (.tenum enumName))}
        cont retTypes) :
    typecheck_stmt lenv env (.letBind b (.packVariant enumName variantName fields) cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj_σ, hnonroot, hpaths_σ, henum_equiv⟩ := hsub
  apply typecheck_stmt.let_bind_packVariant
  · exact SiteEnvSubstEquiv_notIn σ _ _ _ hse hnotIn
  · rw [← henum_equiv]; exact hlookup_enum
  · exact hlookup_var
  · intro f a hmem
    obtain ⟨bt, hlook_a, hlook_f⟩ := hft f a hmem
    have hlook_a_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_a
  ...
  · apply ih
    · exact ⟨σ, hid, hve,
        SiteEnvSubstEquiv_insert σ _ _ _ _ _
          (SiteEnvSubstEquiv_deleteAll σ _ _ _ hse) (applySubstMoveType_basic σ _),
        hrefs, hinj_σ, hnonroot, hpaths_σ, henum_equiv⟩
    · exact hfuneq
    · exact TypeEnv.deleteAll_insert_wf _ _ _ _ hwfL trivial
    · exact TypeEnv.deleteAll_insert_wf _ _ _ _ hwfE trivial
    · exact site_tracked_insert_basic _ _ _ _
...
private theorem weaken_unpackVariant_rule
    (lenv : LabelEnv) (envL env : TypeEnv)
    (variantName : Id) (fields : List (Field × Site)) (b : Site)
    (ename : Id) (enumDef : EnumDef) (variantDef : EnumVariantDef)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_b : lookup envL.siteEnv b = some (.basic (.tenum ename)))
    (hlookup_enum : AssocMap.lookup envL.enumEnv ename = some enumDef)
    (hlookup_var : AssocMap.lookup enumDef.variants variantName = some variantDef)
    (hfresh : ∀ f a, (f, a) ∈ fields → AssocMap.notIn envL.siteEnv a)
    (hinj_f : ∀ a₁ a₂, (∃ f₁ f₂, (f₁, a₁) ∈ fields ∧ (f₂, a₂) ∈ fields ∧ f₁ ≠ f₂) → a₁ ≠ a₂)
    (hexist : ∀ f a, (f, a) ∈ fields → ∃ bt, AssocMap.lookup variantDef.fields f = some bt)
    (hsub : TypeEnv.subsumes envL env)
    (hfuneq : envL.funEnv = env.funEnv)
    (hwfL : TypeEnv.WellFormed envL) (hwfE : TypeEnv.WellFormed env)
    (hsite_tracked : ∀ s bt r bk, lookup envL.siteEnv s = some (.ref bt r bk) → r ∈ envL.pathEnv.refs)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        {envL with siteEnv := addFieldSites variantDef.fields (delete envL.siteEnv b) fields}
        cont retTypes) :
    typecheck_stmt lenv env (.unpackVariant variantName fields b cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj_σ, hnonroot, hpaths_σ, henum_equiv⟩ := hsub
  have hlook_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_b
  simp only [applySubstMoveType] at hlook_env
  apply typecheck_stmt.unpackVariant_rule lenv env _ _ _ _ _ _ _ _
    hlook_env (by rw [← henum_equiv]; exact hlookup_enum) hlookup_var
  · intro f a hmem
    exact SiteEnvSubstEquiv_notIn σ _ _ _ hse (hfresh f a hmem)
  · exact hinj_f
  · exact hexist
...
private theorem weaken_unpackVariant_ref_rule
    (lenv : LabelEnv) (envL env : TypeEnv)
    (variantName : Id) (fields : List (Field × Site)) (b : Site)
    (ename : Id) (enumDef : EnumDef) (variantDef : EnumVariantDef)
    (r : Aref) (bk : BorrowingKind)
    (cont : Stmt) (retTypes : List ParamType)
    (hlook_b : lookup envL.siteEnv b = some (.ref (.tenum ename) r bk))
    (hlookup_enum : AssocMap.lookup envL.enumEnv ename = some enumDef)
    (hlookup_var : AssocMap.lookup enumDef.variants variantName = some variantDef)
    (hfresh : ∀ f a, (f, a) ∈ fields → AssocMap.notIn envL.siteEnv a)
    (hinj_f : ∀ a₁ a₂, (∃ f₁ f₂, (f₁, a₁) ∈ fields ∧ (f₂, a₂) ∈ fields ∧ f₁ ≠ f₂) → a₁ ≠ a₂)
    ...
    (hroot : Aref.root ∈ envL.pathEnv.refs)
    (ih : WeakenIH lenv
        (addRefFieldSites r bk variantDef.fields (qualifyField variantName) fields {envL with siteEnv := delete envL.siteEnv b})
        cont retTypes) :
    typecheck_stmt lenv env (.unpackVariant variantName fields b cont) retTypes := by
  obtain ⟨σ, hid, hve, hse, hrefs, hinj_σ, hnonroot, hpaths_σ, henum_equiv⟩ := hsub
  have hlook_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_b
  ...
  · rw [← henum_equiv]; exact hlookup_enum
  · exact hlookup_var
  · exact fun f a hmem => SiteEnvSubstEquiv_notIn σ _ _ _ hse (hfresh f a hmem)
  · exact hinj_f
  · exact hexist
  · have hr_mem := hsite_tracked _ _ _ _ hlook_b
    have hσr_mem : σ r ∈ env.pathEnv.refs :=
      hrefs.mem_iff.mp (List.mem_map.mpr ⟨r, hr_mem, rfl⟩)
    apply ih
    · exact addRefFieldSites_subsumes σ r (σ r) bk variantDef.fields (qualifyField variantName) fields
        {envL with siteEnv := delete envL.siteEnv b}
        {env with siteEnv := delete env.siteEnv b}
        hid hve (SiteEnvSubstEquiv_delete σ _ _ _ hse)
        hrefs hinj_σ hnonroot hpaths_σ rfl hr_mem
        (site_tracked_delete _ _ _ hsite_tracked) hvar_tracked henum_equiv
    · rw [addRefFieldSites_funEnv_eq, addRefFieldSites_funEnv_eq]; exact hfuneq
    · exact ⟨addRefFieldSites_pathEnv_wf _ _ _ _ _ _ hwfL.pathEnv_wf,
             addRefFieldSites_siteEnv_wf _ _ _ _ _ _
               (SiteEnv.delete_refs_not_root _ _ hwfL.siteEnv_wf),
             by rw [addRefFieldSites_varEnv_eq]; exact hwfL.varEnv_wf⟩
    · exact ⟨addRefFieldSites_pathEnv_wf _ _ _ _ _ _ hwfE.pathEnv_wf,
             addRefFieldSites_siteEnv_wf _ _ _ _ _ _
               (SiteEnv.delete_refs_not_root _ _ hwfE.siteEnv_wf),
             by rw [addRefFieldSites_varEnv_eq]; exact hwfE.varEnv_wf⟩
    · exact addRefFieldSites_site_tracked _ _ _ _ _ _
        (site_tracked_delete _ _ _ hsite_tracked) hr_mem
    · exact addRefFieldSites_var_tracked _ _ _ _ _ _ hvar_tracked
    · exact addRefFieldSites_RefsUnique _ _ _ _ _ _
        (RefsUnique_delete_site _ _ _ huniq)
    · exact addRefFieldSites_paths_to_nm _ _ _ _ _ _ hpaths_to_nm hσr_mem
    · exact addRefFieldSites_paths_from_nm _ _ _ _ _ _ hpaths_from_nm hσr_mem
    · exact addRefFieldSites_self_loop_only_empty _ _ _ _ _ _ hself_loop_only_empty
    · exact addRefFieldSites_root_in_refs _ _ _ _ _ _ hroot

private theorem weaken_variantSwitch_rule
    (lenv : LabelEnv) (envL env : TypeEnv)
    (src : Site) (cases : List (Id × Label))
    (ename : Id) (enumDef : EnumDef) (r : Aref) (bk : BorrowingKind)
    (retTypes : List ParamType)
    (hlook_src : lookup envL.siteEnv src = some (.ref (.tenum ename) r bk))
    (hlookup_enum : AssocMap.lookup envL.enumEnv ename = some enumDef)
    (hcoverage : ∀ vname, AssocMap.lookup enumDef.variants vname ≠ none → ∃ label, (vname, label) ∈ cases)
    (hcases : ∀ vname label, (vname, label) ∈ cases →
  ...
  have hdel_sub : TypeEnv.subsumes
      ...
      {env with siteEnv := delete env.siteEnv src,
                pathEnv := delete_ref_node env.pathEnv (σ r)} := by
    refine ⟨σ, hid, hve,
      SiteEnvSubstEquiv_delete σ _ _ _ hse,
      ?_, ?_, hnonroot, ?_, henum_equiv⟩
    · simp only [delete_ref_node]
      rw [map_filter_ne σ _ r hinj_σ hr_mem]
      exact hrefs.filter _
    · intro u v hu hv
      simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq] at hu hv
      exact hinj_σ u v hu.1 hv.1
    · intro u v hu hv path hinterp
      simp only [delete_ref_node, List.mem_filter, decide_eq_true_eq] at hu hv
      have hσu_ne : σ u ≠ σ r := fun h => hu.2 (hinj_σ u r hu.1 hr_mem h)
      have hσv_ne : σ v ≠ σ r := fun h => hv.2 (hinj_σ v r hv.1 hr_mem h)
      simp only [delete_ref_node] at hinterp ⊢
      rw [if_neg (fun ⟨h, _⟩ => hu.2 h), if_neg (not_or.mpr ⟨hu.2, hv.2⟩)]
      rw [if_neg (fun ⟨h, _⟩ => hσu_ne h), if_neg (not_or.mpr ⟨hσu_ne, hσv_ne⟩)] at hinterp
      exact hpaths_σ u v hu.1 hv.1 path hinterp
  apply typecheck_stmt.variantSwitch_rule lenv env _ _ _ _ (σ r) _ _
    hlook_env (by rw [← henum_equiv]; exact hlookup_enum) hcoverage
  · intro vname label hmem
    obtain ⟨envJ, hlookup_j, hsubJ⟩ := hcases vname label hmem
    exact ⟨envJ, hlookup_j, TypeEnv.subsumes_trans _ _ _ hsubJ hdel_sub⟩
...
theorem typecheck_stmt_weaken (lenv : LabelEnv) (envL env : TypeEnv) (s : Stmt) (retTypes : List ParamType)
  ...
  | branch envL' a _ _ _ _ _ hsite hl1 hl2 hs1 hs2 =>
    ...
    have hdel_sub : TypeEnv.subsumes
      ...
      ⟨σ, hid, hve, SiteEnvSubstEquiv_delete σ _ _ a hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩
  ...
  | ret _ _ _ hconf hnolocal hwritable hnoalias =>
    exact weaken_ret lenv _ env _ _ hconf hnolocal hwritable hnoalias hsub hroot hsite_tracked hself_loop_only_empty
  | abort _ _ _ _ hlook =>
  ...
  | var_assign_invalid _ _ _ _ _ _ _ hlook_x hlook_a hcompat _ ih =>
    exact weaken_var_assign_invalid lenv _ env _ _ _ _ _ _
      ...
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | var_assign_overwrite_basic envL_pat x_pat a_pat isv_pat τ_pat cont_pat _ hlook_x_pat hlook_a_pat _ ih_pat =>
    obtain ⟨σ, hid, hve, hse, hrefs, hinj, hnonroot, hpaths, henum_equiv⟩ := hsub
    have hlook_a_env := SiteEnvSubstEquiv_lookup_some σ _ _ _ _ hse hlook_a_pat
    simp only [applySubstMoveType] at hlook_a_env
    -- Get env's entry for x: in all cases, type stays .basic τ_pat
    have ⟨isv2, hlook_x_env⟩ : ∃ isv2, lookup env.varEnv x_pat = some (isv2, .basic τ_pat, .mutable) := by
      cases isv_pat with
      | validVar =>
        have h := VarEnvSubstEquiv_lookup_valid σ _ _ _ _ _ hve hlook_x_pat
        simp only [applySubstMoveType] at h
        exact ⟨.validVar, h⟩
      | invalidVar =>
        obtain ⟨isv2, τ2, h, hbc⟩ := VarEnvSubstEquiv_lookup_invalid σ _ _ _ _ _ hve hlook_x_pat
        cases τ2 with
        | basic bt2 =>
          simp [MoveType.baseCompatible] at hbc; subst hbc; exact ⟨isv2, h⟩
        | ref => simp [MoveType.baseCompatible] at hbc
    apply typecheck_stmt.var_assign_overwrite_basic lenv env _ _ isv2 _ _ _ hlook_x_env hlook_a_env
    apply ih_pat
    · exact ⟨σ, hid,
        VarEnvSubstEquiv_update_valid σ _ _ _ _ _ hve,
        SiteEnvSubstEquiv_delete σ _ _ _ hse,
    ...
    · exact ⟨hwfE.pathEnv_wf,
            ...
            VarEnv.update_refs_not_root _ _ _ hwfE.varEnv_wf (by simp)⟩
    · exact site_tracked_delete _ _ _ hsite_tracked
    · exact var_tracked_update_valid_from_site _ _ _ _ _ _ _ hlook_a_pat hsite_tracked hvar_tracked
    · exact RefsUnique_assign_from_site _ _ _ _ _ _ hlook_a_pat huniq
    · exact hpaths_to_nm
    · exact hpaths_from_nm
    · exact hself_loop_only_empty
    · exact hwfL.pathEnv_wf.root_in_refs
  ...
  | release _ a _ r _ _ _ hlook_a _ ih =>
    exact weaken_release lenv _ env a _ r _ _ _ hlook_a
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | release_basic _ a bt _ _ hlook_a _ ih =>
    exact weaken_release_basic lenv _ env a bt _ _ hlook_a
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  ...
  | let_bind_borrowMut envL a x_var _ _ r _ _ hle hlookup hnotIn hfresh _ ih =>
    exact weaken_let_bind_borrowMut lenv envL env a x_var _ _ r _ _
      hle hlookup hnotIn hfresh hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  ...
  | var_assign_valid_ref _ _ _ _ _ _ _ _ _ _ hle hlook_x hlook_a _ ih =>
    exact weaken_var_assign_valid_ref lenv _ env _ _ _ _ _ _ _ _ _
      hle hlook_x hlook_a
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  ...
  | let_bind_vecPack _ _ _ _ _ _ hnotIn hforall hnodup _ ih =>
    exact weaken_let_bind_vecPack lenv _ env _ _ _ _ _ hnotIn hforall hnodup
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | vecUnpack_rule _ _ _ _ _ _ hlook hfresh hnodup _ ih =>
    exact weaken_vecUnpack_rule lenv _ env _ _ _ _ _ hlook hfresh hnodup
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | let_bind_vecLen _ _ _ _ _ _ _ _ hlook hnotIn _ ih =>
    exact weaken_let_bind_vecLen lenv _ env _ _ _ _ _ _ _ hlook hnotIn
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | let_bind_vecImmBorrow _ _ _ _ _ _ _ _ _ _ hlook_src hlook_idx hnotIn houtbound hfresh _ ih =>
    exact weaken_let_bind_vecImmBorrow lenv _ env _ _ _ _ _ _ _ _ _ hlook_src hlook_idx hnotIn houtbound hfresh
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | let_bind_vecMutBorrow _ _ _ _ _ _ _ _ _ hlook_src hlook_idx hnotIn houtbound hfresh _ ih =>
    exact weaken_let_bind_vecMutBorrow lenv _ env _ _ _ _ _ _ _ _ hlook_src hlook_idx hnotIn houtbound hfresh
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | let_bind_vecPopBack _ _ _ _ _ _ _ hlook houtbound hnotIn _ ih =>
    exact weaken_let_bind_vecPopBack lenv _ env _ _ _ _ _ _ hlook houtbound hnotIn
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | vecPushBack_rule _ _ _ _ _ _ _ hlook_ref hlook_val houtbound _ ih =>
    exact weaken_vecPushBack_rule lenv _ env _ _ _ _ _ _ hlook_ref hlook_val houtbound
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | vecSwap_rule _ _ _ _ _ _ _ _ hlook_ref hlook_idx1 hlook_idx2 houtbound _ ih =>
    exact weaken_vecSwap_rule lenv _ env _ _ _ _ _ _ _ hlook_ref hlook_idx1 hlook_idx2 houtbound
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  -- Enum cases
  | let_bind_packVariant _ _ _ _ _ _ _ _ _ hnotIn hlookup_enum hlookup_var hft hfc hfi _ ih =>
    exact weaken_let_bind_packVariant lenv _ env _ _ _ _ _ _ _ _ hnotIn hlookup_enum hlookup_var hft hfc hfi
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | unpackVariant_rule _ _ _ _ _ _ _ _ _ hlook_b hlookup_enum hlookup_var hfresh hinj_f hexist _ ih =>
    exact weaken_unpackVariant_rule lenv _ env _ _ _ _ _ _ _ _ hlook_b hlookup_enum hlookup_var hfresh hinj_f hexist
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih
  | unpackVariant_ref_rule _ _ _ _ _ _ fentries_ur r_ur bk_ur _ _ hlook_b hlookup_enum hlookup_var hfresh hinj_f hexist _ ih_cont =>
    exact weaken_unpackVariant_ref_rule lenv _ env _ _ _ _ _ fentries_ur r_ur bk_ur _ _ hlook_b hlookup_enum hlookup_var hfresh hinj_f hexist
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot ih_cont
  | variantSwitch_rule _ src _ _ _ r _ _ hlook_src hlookup_enum hcoverage hcases =>
    exact weaken_variantSwitch_rule lenv _ env _ _ _ _ r _ _ hlook_src hlookup_enum hcoverage hcases
      hsub hfuneq hwfL hwfE hsite_tracked hvar_tracked huniq
      hpaths_to_nm hpaths_from_nm hself_loop_only_empty hroot
