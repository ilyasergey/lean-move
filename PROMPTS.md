...
recollection of the human prover, and each block is annotated with the dates and
the commits it corresponds to so the correspondence can be checked against the
record. They reflect the *style and granularity* of the actual interaction —
natural-language steering at the level of goals, decompositions, and design
decisions, not individual tactics. Two of them (marked *verbatim*) are quoted
...
*(Dec 18, 2025 — commits `fcc6d94` jump/branch rules, `b3c278d` basic blocks +
LabelEnv, `dccc899` return rule checks no locals borrowed, `aac412a`
`typecheck_fun`, `181c617` freeze consumes the old reference.)*
...
*(Jan 21, 2025 — commits `1f8dbe7`, `1d448b2`.)*
...
*(Jan 22, 2025 — commits `350691c`, `5953dd6`, `1ed61ab`.)*
...
*(Feb 12 — commits `6965b5c` semantics, `f44f161` soundness scaffold.)*
...
*(Feb 13 — commits `30d1452` preservation plan, `a0d9a65` inversion lemmas,
`98e76d4` extract cases, `35b1ad9` release case, `0b33d5a`/`4e4cc43` assign
cases, `7079823` borrowImm.)*
...
*(Feb 13 — commits `2e80b9f`, `6e2b87b`.)*
...
*(Feb 13 — commit `2a54fa8`; further clauses 18–20 on Feb 15, commit `330d78b`.)*

> Make environment well-formedness decidable so the example files stop carrying
> manual well-formedness proofs.

*(Feb 14 — commit `cc1ca2c`; also `85a441c` `initState_safe` + `SoundnessAssumptions`,
`4d63d8f` `type_soundness_dec`.)*

> Extract the repeated reasoning in these preservation cases into reusable
> lemmas, then simplify the cases that use them.

*(Feb 14 — commits `0617b4f` "extract 7 reusable lemmas, simplify 6 cases",
`486e6e7` "extract 5 more, simplify 4".)*
...
*(Feb 16 — commit `5ca67cd` "Prove preservation_writeRef (sorry-free)"; `a52146a`
unpack.)*
...
*(Feb 6 — commit `1095e1f` "Fix reversed update_with_extension args in assign
rule and prove assign case".)*
...
*(Feb 16–18 — commits `8706f9d` weakening infra, `a6bc53a` "extract 6 large
cases into lemmas", `ec5c166` extract remaining cases, `2fef5da` "Weakening.lean
is now sorry-free".)*
...
*(Feb 17 — commits `0889d7c` "Fix call typing rule: bidirectional
extend_with_star, output population", `04c6507` "swap extend_with_star args,
remove overly restrictive check, add tests".)*
...
*(Feb 18–19 — commits `9bad89f` ret refactor, `b0720eb` ret return conditions,
`b2a4183` `preservation_ret`, `e996b66` "Prove preservation_call: eliminate all
sorrys", `cac8b1d` "Eliminate last sorry".)*
...
*(Feb 18 — commit `5c17f6b`, whose message notes the manual commit during an
outage.)*
...
*(Feb 22 — commits `ae15f38` parser + pretty-printer, `cd97a0b`/`fce13f0`
alpha-equivalence tests, `44a922c` test-env simplification, `5ba0980` "rule out
all 8 preventable runtime errors", `b28814c` "Consume call input sites from
siteEnv; refine ret writability check".)*
...
*(Mar 3–4 — commits `88030f4`…`86bbed4` phases 1–9, `0704c69` rejection tests
from the Sui bytecode-verifier suite, `edc0a4e` "Complete type soundness proof
for vector extension (zero sorrys)".)*
...
*(Mar 5–10 — commits `f6ec042` stubs, `43a9270`/`e318d83`/`2e8c9ec` wip,
`49ec320` complete enum support, `2588285` enum invariant + InitState,
`4a9ad0b` "Multi-variant enum soundness: flat encoding, zero sorrys, decidable
certificate".)*
...
> Add a Makefile with a `zip-anon` target that archives the committed tree,
> drops the Makefile itself, and `sed`-scrubs identifying strings from the
> `.lean` files; exclude the working-notes directory from the archive.

*(Mar 13–15 — commits `25cd1db`, `092039a`, `32f73fe`, `650fb4c`.)*
